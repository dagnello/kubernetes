...
func (lbaas *LbaasV2) EnsureLoadBalancer(apiService *api.Service, hosts []string, annotations map[string]string) (*api.LoadBalancerStatus, error) {
	...
	ports[0].Port = 3306
	ports[0].NodePort = 3306
	ports[0].Protocol = "TCP" //delete
	if len(ports) > 1 {
...
func (lbaas *LbaasV2) UpdateLoadBalancer(service *api.Service, hosts []string) error {
	...
	ports = append(ports, api.ServicePort{}) //delete
	ports[0].NodePort = 3306
	if len(ports) > 1 {
...
func (lb *LbaasV1) EnsureLoadBalancer(apiService *api.Service, hosts []string, annotations map[string]string) (*api.LoadBalancerStatus, error) {
	glog.V(4).Infof("EnsureLoadBalancer(%v, %v, %v, %v, %v, %v)", apiService.Namespace, apiService.Name, apiService.Spec.LoadBalancerIP, apiService.Spec.Ports, hosts, annotations)
